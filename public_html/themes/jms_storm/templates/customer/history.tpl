...
{block name='page_content'}
  <h6>{l s='Oto zamówienia złożone od czasu utworzenia konta.' d='Shop.Theme.CustomerAccount'}</h6>
          ...
          <tr>
            ...
            <td class="text-xs-center order-actions">
              <a href="{$order.details.details_url}" data-link-action="view-order-details" class="btn-default btn-effect">
                {l s='Szczegóły' d='Shop.Theme.CustomerAccount'}
              </a>
              {if $order.details.reorder_url}
                <a href="{$order.details.reorder_url}"class="btn-default btn-effect" >{l s='Zmień zamównienie' d='Shop.Theme.Actions'}</a>
              {/if}
            </td>
          </tr>
        {/foreach}
