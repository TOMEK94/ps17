...
<script type='text/javascript'>
	$(document).ready(function() {
		var height = {$promobar_height nofilter};
...
<div class="promobar{if $box_class} {$box_class nofilter}{/if}{if $promobar_position == top} top{else} bottom{/if}{if $promobar_fixed == 'yes'} promobar-fixed{else}{/if}" style="{if $promobar_bg}background-color: {$promobar_bg nofilter};{/if}{if $promobar_height} height: {$promobar_height nofilter}px;{/if}">
	{$promobar_message nofilter}
	{if $promobar_expire_time}
	<span class="promo-countdown">
		{$promobar_expire_time nofilter}
	</span>
	{/if}
