...
{if isset($posts) AND $posts}		
	<div class="post-list categorylist post-4item">
		{foreach from=$posts item=post}
			...
			<article class="blog-post">							
				...
				<ul class="post-meta">
					...
					{/if}	
					<li class="title-blog"><a href="{jmsblog::getPageLink('jmsblog-post', $params) nofilter}">{$post.title nofilter}</a></li>					
					<li class="post-created">
						<span class="lnr lnr-calendar-full"></span>
						{$post.created|date_format:"%B %e, %Y" nofilter}
					</li>
					{if $jmsblog_setting.JMSBLOG_SHOW_VIEWS}
					<li class="post-views">
						<span class="lnr lnr-eye"></span>
						<span>{l s='Views' d='Modules.JmsBlog'} :</span> {$post.views nofilter}
					</li>
					{/if}
					{if $jmsblog_setting.JMSBLOG_SHOW_COMMENTS}
					<li class="post-comments">
						<span class="lnr lnr-users"></span>
						<span>{l s='Comments' d='Modules.JmsBlog'} :</span> {$post.comment_count nofilter}
					</li>
					{/if}	
				</ul>
				<div class="blog-intro">{$post.introtext|truncate: 150 :"..." nofilter}</div>				
				<a class="btn btn-default blog-readmore button-small"  href="{jmsblog::getPageLink('jmsblog-post', $params)|replace:'amp;':''}">{l s='Read more' d='Modules.JmsBlog'}</a>								
			</article>			
		{/foreach}
	</div>
{else}	
