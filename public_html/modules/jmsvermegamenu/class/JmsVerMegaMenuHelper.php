...
class JmsVerMegaMenuHelper
{
    ...
    public function tree($parent, $ident, $tree, $current_id = 0)
    {
        ...
        $sql = 'SELECT a.*,b.name FROM '._DB_PREFIX_.'jmsvermegamenu AS a ';
        $sql .= 'INNER JOIN '._DB_PREFIX_.'jmsvermegamenu_lang AS b ON a.mitem_id = b.mitem_id ';
    ...
    public function delptree($parent, $level, $tree, $status = '')
    {
        $context = Context::getContext();
        $id_lang = $context->language->id;
        $sql = 'SELECT a.*,b.name,b.`menulink` FROM '._DB_PREFIX_.'jmsvermegamenu AS a ';
        $sql .= 'INNER JOIN '._DB_PREFIX_.'jmsvermegamenu_lang AS b ON a.mitem_id = b.mitem_id ';
    ...
    public function getParentList($current_parent = 0, $mitem_id = 0)
    {
        ...
        $id_lang = $context->language->id;
        $sql = 'SELECT *';
        $sql .= '   FROM '._DB_PREFIX_.'jmsvermegamenu AS a';
        $sql .= '   INNER JOIN '._DB_PREFIX_.'jmsvermegamenu_lang AS b  ';
    ...
    {
        $sql = 'SELECT ordering FROM '._DB_PREFIX_.'jmsvermegamenu ';
